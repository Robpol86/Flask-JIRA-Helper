...
class PyTest(test):
    TEST_ARGS = ['--cov-report', 'term-missing', '--cov', 'flask_jira', 'tests']
...
class PyTestCovWeb(PyTest):
    TEST_ARGS = ['--cov-report', 'html', '--cov', 'flask_jira', 'tests']
...
class CmdLint(CmdFlake):
    CMD_ARGS = ['pylint', '--max-line-length', '120', 'flask_jira.py']
...
setuptools.setup(
    ...
    **get_metadata(os.path.join('flask_jira.py'))
)
